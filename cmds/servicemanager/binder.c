...
void binder_dump_txn(struct binder_transaction_data *txn)
{
    ...
    fprintf(stderr,"  target %016llx  cookie %016llx  code %08x  flags %08x\n",
            (uint64_t)txn->target.ptr, (uint64_t)txn->cookie, txn->code, txn->flags);
    fprintf(stderr,"  pid %8d  uid %8d  data %llu  offs %llu\n",
    ...
    while (count--) {
        ...
        fprintf(stderr,"  - type %08x  flags %08x  ptr %016llx  cookie %016llx\n",
...
/* Note: The payload passed to BC_REQUEST_DEATH_NOTIFICATION does not match
 *      the structure used to defined this command. I.e. the payload should
 *      have been struct binder_ptr_cookie, but we use:
 *          struct foo {
 *              uint32_t handle;
 *              void *ptr;
 *          }
